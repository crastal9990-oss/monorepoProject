...
import { X } from 'lucide-react' // 引入一个关闭图标用于预览弹窗

export interface EditorProps {
  content: string;
  onChange: (content: string) => void;
  uploadFn?: (file: File) => Promise<string | null>; // 接收一个 uploadFn 作为 prop，让业务层自己决定怎么上传
  className?: string;
  placeholder?: string;
}
...
export function Editor({ content, onChange, uploadFn, className, placeholder }: EditorProps) {
  ...
  const { handlePaste, handleDrop } = useImageUpload(uploadFn) // 使用自定义 Hook 接管图片上传相关的事件监听

  const [previewImage, setPreviewImage] = useState<string | null>(null) // 用于控制图片预览弹窗的状态
  // 解决滚动穿透问题的副作用钩子
  useEffect(() => {
    if (previewImage) {
      document.body.style.overflow = 'hidden' // 弹窗打开时，锁定 body 滚动
    } else {
      document.body.style.overflow = '' // 弹窗关闭时，恢复 body 滚动
    }
    // 极其重要的清理函数：防止组件在弹窗打开的状态下被卸载，导致整个应用永远无法滚动
    return () => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({
        inline: true,
        HTMLAttributes: {
          class: 'rounded-lg max-w-full mx-auto border border-border/50 align-middle cursor-zoom-in hover:ring-2 hover:ring-primary hover:ring-offset-2 hover:ring-offset-background transition-all',// 允许图片响应式调整大小
        },
      }),
    ],
    content,
    editorProps: {
      ...
      handleClickOn: (view, pos, node, nodePos, event, direct) => {
        if (node.type.name === 'image') {
          setPreviewImage(node.attrs.src)// 获取图片的 src 属性并将其存入状态，触发预览
          return true// 返回 true 告诉 Tiptap 我们已经接管了这个点击事件，阻止默认行为
        }
        return false
      }
    },
